...
this.Revolution();
  ...
  $(document).ready(function () {
    ...
    // Bind directly (not via document delegation) so the stopPropagation on
    // .clv_menu_nav never intercepts the click before our handler runs.
    $('.clv_menu_nav > ul > li > .nav-arrow').on('click', function (e) {
      console.log('clicked');
      e.stopPropagation(); // prevents nav from closing on this click
      /*	if ($(window).width() > 991) { return; }
        var $li = $(this).closest('li');
        $li.siblings('.menu-open').children('ul').slideUp(200);
