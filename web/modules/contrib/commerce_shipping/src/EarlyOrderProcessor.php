...
class EarlyOrderProcessor implements OrderProcessorInterface {
  ...
  protected function shouldRepack(OrderInterface $order, array $shipments) {
    ...
    if (DeprecationHelper::backwardsCompatibleCall(\Drupal::VERSION, '11.2.0', fn() => $order->getOriginal() !== NULL, fn() => isset($order->original)) && $order->hasField('checkout_step')) {
      $previous_step = DeprecationHelper::backwardsCompatibleCall(\Drupal::VERSION, '11.2.0', fn() => $order->getOriginal(), fn() => $order->original)->get('checkout_step')->value;
      $current_step = $order->get('checkout_step')->value;
      $previous_order_item_ids = array_map(function ($value) {
        return $value['target_id'];
      }, DeprecationHelper::backwardsCompatibleCall(\Drupal::VERSION, '11.2.0', fn() => $order->getOriginal(), fn() => $order->original)->get('order_items')->getValue());
  ...
  protected function shouldRefresh(OrderInterface $order) {
    if ($order->getData(ShippingOrderManagerInterface::FORCE_REFRESH, FALSE)) {
      return TRUE;
    }

    // Return FALSE for new or non-draft orders.
    if ($order->isNew() || $order->getState()->getId() !== 'draft') {
      return FALSE;
    }

    // When there is no original order we don't have anything to compare with.
    $original_order = DeprecationHelper::backwardsCompatibleCall(\Drupal::VERSION, '11.2.0', fn() => $order->getOriginal(), fn() => $order->original);
    if (!($original_order instanceof OrderInterface)) {
      return FALSE;
    }

    // When the list of items is changed we need to repack and recalculated
    // shipments.
    $item_ids = array_map(function ($item) {
      return $item->id();
    }, $order->getItems());
    $original_item_ids = array_map(function ($item) {
      return $item->id();
    }, $original_order->getItems());

    // Filter new order items.
    $item_ids = array_filter($item_ids);
    $original_item_ids = array_filter($original_item_ids);
    if (count($item_ids) !== count($original_item_ids)
      || array_diff($item_ids, $original_item_ids)
    ) {
      return TRUE;
    }
