...
class ShipmentRouteProvider extends AdminHtmlRouteProvider {
  ...
  public function getRoutes(EntityTypeInterface $entity_type) {
    ...
    if ($edit_modal_form_route = $this->getEditModalFormRoute($entity_type)) {
      $collection->add("entity.commerce_shipment.edit_modal_form", $edit_modal_form_route);
    }

    if ($add_modal_form_route = $this->getAddModalFormRoute($entity_type)) {
      $collection->add("entity.commerce_shipment.add_modal_form", $add_modal_form_route);
    }
  ...
  /**
   * Gets the edit-modal-form route.
   *
   * @param \Drupal\Core\Entity\EntityTypeInterface $entity_type
   *   The entity type.
   *
   ...
   *   The generated route, if available.
   */
  protected function getEditModalFormRoute(EntityTypeInterface $entity_type) {
    $route = new Route($entity_type->getLinkTemplate('edit-modal-form'));
    $route
      ->addDefaults([
        '_entity_form' => 'commerce_shipment.edit-modal',
        '_title_callback' => '\Drupal\Core\Entity\Controller\EntityController::editTitle',
      ])
      ->setRequirement('_entity_access', 'commerce_shipment.update')
      ->setRequirement('commerce_shipment', '\d+')
      ->setOption('parameters', [
        'commerce_shipment' => [
          'type' => 'entity:commerce_shipment',
   ...
   *   The generated route, if available.
   */
  protected function getAddModalFormRoute(EntityTypeInterface $entity_type) {
    $route = new Route($entity_type->getLinkTemplate('add-modal-form'));
    $route
      ->addDefaults([
        '_entity_form' => 'commerce_shipment.add-modal',
        '_title' => 'Add shipping information',
        'entity_type_id' => 'commerce_shipment',
        'bundle_parameter' => 'commerce_shipment_type',
      ])
      ->setRequirement('_entity_create_access', 'commerce_shipment:{commerce_shipment_type}')
      ->setOption('parameters', [
        'commerce_order' => [
          'type' => 'entity:commerce_order',
        ],
