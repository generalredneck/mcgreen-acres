...
class PromotionSubscriber implements EventSubscriberInterface {
  ...
  public function onCalculate(ShippingRatesEvent $event): void {
    ...
    // Remove already applied shipping adjustment to properly get applicable promotions.
    $adjustments = $fake_order->getAdjustments();
    if ($adjustments) {
      $adjustments = array_filter($adjustments, function ($adjustment) {
        return $adjustment->getType() !== 'shipping';
      });
      $fake_order->setAdjustments($adjustments);
    }
