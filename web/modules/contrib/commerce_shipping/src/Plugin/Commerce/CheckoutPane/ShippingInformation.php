...
class ShippingInformation extends CheckoutPaneBase implements ContainerFactoryPluginInterface {
  ...
  public function buildPaneForm(array $pane_form, FormStateInterface $form_state, array &$complete_form) {
    return $this->executeWithOrderLoadedForUpdate(function () use ($pane_form, $form_state, &$complete_form) {
      return $this->doBuildPaneForm($pane_form, $form_state, $complete_form);
    });
  }

  /**
   * Builds the shipping information pane form.
   *
   * @see \Drupal\commerce_shipping\Plugin\Commerce\CheckoutPane\ShippingInformation::buildPaneForm()
   */
  protected function doBuildPaneForm(array $pane_form, FormStateInterface $form_state, array &$complete_form) {
  ...
  public function validatePaneForm(array &$pane_form, FormStateInterface $form_state, array &$complete_form) {
    return $this->executeWithOrderLoadedForUpdate(function () use (&$pane_form, $form_state, &$complete_form) {
      return $this->doValidatePaneForm($pane_form, $form_state, $complete_form);
    });
  }

  /**
   * Validates the shipping information pane form.
   *
   * @see \Drupal\commerce_shipping\Plugin\Commerce\CheckoutPane\ShippingInformation::validatePaneForm()
   */
  protected function doValidatePaneForm(array &$pane_form, FormStateInterface $form_state, array &$complete_form) {
  ...
  /**
   * Runs a callback with the order reloaded and locked for update.
   *
   * The shipping pane can save the order while (re)building or validating the
   * form, which races with concurrent checkout AJAX requests and triggers an
   * OrderVersionMismatchException. Reloading the order under a lock keeps the
   * in-memory order in sync with the stored one for the duration of the
   * callback; the lock is released here for the code paths that acquire it
   * without saving (OrderStorage::save() releases it otherwise).
   *
   ...
   */
  protected function executeWithOrderLoadedForUpdate(callable $callback): mixed {
    /** @var \Drupal\commerce_order\OrderStorageInterface $order_storage */
    $order_storage = $this->entityTypeManager->getStorage('commerce_order');
    if ($order = $order_storage->loadForUpdate((int) $this->order->id())) {
      $this->order = $order;
    }
    try {
      return $callback();
      ...
      if ($this->order->id()) {
        $order_storage->releaseLock((int) $this->order->id());
      }
    }
  }
