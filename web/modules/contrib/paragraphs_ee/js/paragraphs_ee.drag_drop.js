(function ($, Drupal, drupalSettings, once) {
  ...
  const arrowClick = (evt) => {
    ...
    while (swapRow && !swapRow.matches('.draggable')) {
      swapRow =
        direction === 'up'
          ? swapRow.previousElementSibling
          : swapRow.nextElementSibling;
    }

    // Let Assistive Tech users know they cannot move the row this direction.
    if (!swapRow) {
      let message = Drupal.t(
        'You cannot move this row up because it is already the first.',
      );
      if (direction === 'down') {
        message = Drupal.t(
          'You cannot move this row down because it is already the last.',
        );
      }
    ...
    let message = Drupal.t(
      'The row has been moved up. Do not forget to save your changes.',
    );
    if (direction === 'down') {
      message = Drupal.t(
        'The row has been moved down. Do not forget to save your changes.',
      );
    }
    Drupal.announce(message);
    ...
    if (tableDrag.changed === false) {
      ...
      draggableTable.parentNode.insertBefore(
  ...
  Drupal.behaviors.paragraphsEeDragDropArrowsInit = {
    attach(context) {
      once('drag-and-drop-arrows', '.drag-drop-buttons').forEach((el) => {
        ...
        // Attach to tabledrag.
        tabledragHandle.parentNode.insertBefore(
          $arrowUp.get(0),
          tabledragHandle,
        );
  ...
  Drupal.behaviors.paragraphsEeDragDropArrowsEvents = {
    attach(context) {
      once('drag-and-drop-arrow-events', '.js-drag-and-drop-arrow').forEach(
        (el) => {
          // Add event listener.
    ...
    /** @lends Drupal.theme */ {
      ...
      paragraphsEeTableDragArrowDown() {
        return `<a href="#" class="drag-and-drop-arrows__arrow drag-and-drop-arrows__arrow--down js-drag-and-drop-arrow" data-drag-and-drop-direction="down" title="${Drupal.t(
          'Move this @paragraph_title down',
          { '@paragraph_title': drupalSettings.paragraphs_ee.widgetTitle },
        )}"></a>`;
      },
