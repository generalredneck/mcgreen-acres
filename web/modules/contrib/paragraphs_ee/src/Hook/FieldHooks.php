...
class FieldHooks {
  ...
  public function fieldWidgetThirdPartySettingsForm(WidgetInterface $plugin, FieldDefinitionInterface $field_definition, string $form_mode, array $form, FormStateInterface $form_state): array {
    ...
    // Drag&drop arrows are an addition to paragraphs_features' own drag & drop
    // reordering, so offering them makes no sense once that is disabled.
    $drag_drop_related_options_rule = [
      ':input[name="fields[' . $field_definition->getName() . '][settings_edit_form][third_party_settings][paragraphs_features][show_drag_and_drop]"]' => [
        'checked' => TRUE,
      ],
    ];
    $elements['paragraphs_ee']['drag_drop'] = [
      ...
      '#states' => [
        'enabled' => $drag_drop_related_options_rule,
        'visible' => $drag_drop_related_options_rule,
      ],
    ];
   ...
   *
   * @phpstan-param string[]|\Drupal\Core\StringTranslation\TranslatableMarkup[] $summary
   *   The summary texts.
   * @phpstan-param array{'widget': object, ...} $context
   *   Given context.
   */
  ...
  public function fieldWidgetSettingsSummaryAlter(array &$summary, array $context): void {
    ...
    $show_drag_and_drop = (bool) $widget->getThirdPartySetting('paragraphs_features', 'show_drag_and_drop', TRUE);
    if ($show_drag_and_drop && isset($settings['paragraphs_ee']['drag_drop']) && ((bool) $settings['paragraphs_ee']['drag_drop'] === TRUE)) {
