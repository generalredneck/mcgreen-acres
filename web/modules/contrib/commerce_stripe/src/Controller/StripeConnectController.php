  ...
  public function oauthReturn(PaymentGatewayInterface $commerce_payment_gateway, Request $request): RedirectResponse {
    ...
    try {
      ...
      if (isset($response['access_token'], $response['stripe_publishable_key'])) {
        $message = $this->t('Stripe credentials generated. This is the only time they will be displayed. Please copy and store them securely.');
        $html = Markup::create($message . sprintf('<br/><strong>Publishable key:</strong> %s<br/><strong>Access token:</strong> %s<br/><strong>Stripe User ID:</strong> %s', $response['stripe_publishable_key'], $response['access_token'], $response['stripe_user_id'] ?? ''));
        $this->messenger->addWarning($html);
