...
class PrlpController extends UserController {
  ...
  public function prlpResetPassLogin(Request $request, $uid, $timestamp, $hash) {
    ...
    try {
      ...
      if ($response_route && $response_route->getRouteName() == 'entity.user.edit_form') {
        ...
        // Allow other modules to alter the destination route. Keep a copy of
        // the original in case a bad value is returned.
        $original_login_destination = $login_destination;
        $this->moduleHandler()->alter('prlp_login_destination', $login_destination);

        // Get the Url from either the altered or original login destination.
        $internal_redirect_url = $this->getInternalRedirectUrl(
          $login_destination,
          $original_login_destination
        );
        if ($internal_redirect_url) {
          return new RedirectResponse($internal_redirect_url->toString());
        }
        // Fallback to the user entity page.
  ...
  /**
   * Recursively return a Url based on login_destination or the original param.
   *
   * @param string $login_destination
   *   The path string to attempt to turn into a Url.
   * @param string $original
   *   A fallback in case the first parameter fails.
   *
   * @return \Drupal\Core\Url|null
   *   The Url or NULL when it has failed in both cases.
   */
  private function getInternalRedirectUrl(string $login_destination, string $original = ''): ?Url {
    $url = NULL;
    try {
      $url = Url::fromUri('internal:' . $login_destination);
    }
    catch (\InvalidArgumentException $e) {
      // This is the 'user' logger so make message uniquely identify PRLP.
      $this->logger->error('PRLP Invalid login destination: @bad_path', [
        '@bad_path' => $login_destination,
      ]);

      if ($original) {
        return $this->getInternalRedirectUrl($original);
      }
    }
