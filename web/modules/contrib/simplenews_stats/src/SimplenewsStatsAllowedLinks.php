...
class SimplenewsStatsAllowedLinks {
  ...
  public const TABLE_NAME = 'simplenews_stats_allowedlinks';
   ...
   *   The link to add.
   */
  public function add(EntityInterface $entity, string $link): void {
    $this->insert([
  ...
  public function update(array $entry): int {
    ...
    try {
      ...
      $this->messenger()
        ->addMessage($this->t('db_update failed. Message = %message, query= %query',
          [
            '%message' => $e->getMessage(),
   ...
   *
   * @return bool
   *   Return TRUE if already allowed or FALSE.
   */
  public function isLinkExist(EntityInterface $entity, string $link): bool {
    $select = $this->connection
      ->select(static::TABLE_NAME)
