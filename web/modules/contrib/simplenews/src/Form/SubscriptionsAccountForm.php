...
class SubscriptionsAccountForm extends SubscriptionsFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ?UserInterface $user = NULL) {
    // If user has no email address, we can't subscribe, show a message.
    if (!$user->getEmail()) {
      $form['no_email'] = [
        '#markup' => $this->t('Your account has no email address. Please add one to access your newsletter subscriptions.'),
        '#weight' => -1,
      ];
      return $form;
    }
