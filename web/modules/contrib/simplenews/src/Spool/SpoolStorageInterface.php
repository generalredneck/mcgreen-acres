...
interface SpoolStorageInterface {
   ...
   * @param bool $postSave
   *   TRUE if called after the entity has already been fully saved, such as
   *   from hook_ENTITY_TYPE_insert() or hook_ENTITY_TYPE_update(). This
   *   forces an explicit save of the changes made here, since the entity's
   *   own save is no longer in progress to pick them up. Defaults to FALSE,
   *   for calls made on an already-loaded entity outside of a save, such as
   *   from a form submit handler.
   */
