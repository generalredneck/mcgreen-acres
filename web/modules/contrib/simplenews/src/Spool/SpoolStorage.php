...
class SpoolStorage implements SpoolStorageInterface {
  ...
  public function __construct(Connection $connection, LockBackendInterface $lock, ConfigFactoryInterface $config_factory, ModuleHandlerInterface $module_handler, RecipientHandlerManager $recipient_handler_manager) {
    ...
    $this->lock = $lock;
    // Store the factory rather than the config object, so that configuration
    // changed after this service was instantiated is not read stale.
  ...
  public function clear() {
    ...
    $expiration_time = \Drupal::time()->getRequestTime() - $this->configFactory->get('simplenews.settings')->get('mail.spool_expire') * 86400;
   ...
   * {@inheritdoc}
   */
  public function addIssue(ContentEntityInterface $issue, bool $postSave = FALSE) {
    ...
    // During a presave, the changes above are picked up by the save already
    // in progress, so no explicit save is needed here. Once the entity has
    // already been fully saved - for example when called from
    // hook_ENTITY_TYPE_insert() or hook_ENTITY_TYPE_update() - an explicit
    // save is required to persist them; callers in that situation must pass
    // $postSave as TRUE.
    if ($postSave || !isset($issue->original)) {
      // Guard against the save below re-entering this method through the
      // entity's own insert/update hooks.
      if (empty($issue->simplenews_spooling)) {
        $issue->simplenews_spooling = TRUE;
        $issue->save();
        unset($issue->simplenews_spooling);
      }
    }
  ...
  protected function getExpirationTime() {
    $timeout = $this->configFactory->get('simplenews.settings')->get('mail.spool_progress_expiration');
