...
use Drupal\commerce_fee\Plugin\Commerce\Fee\FeeInterface;
...
class FeeManager extends DefaultPluginManager {
  ...
  public function __construct(\Traversable $namespaces, CacheBackendInterface $cache_backend, ModuleHandlerInterface $module_handler, EntityTypeManagerInterface $entity_type_manager) {
    parent::__construct(
      ...
      FeeInterface::class,
      CommerceFee::class,
      'Drupal\commerce_fee\Annotation\CommerceFee'
    );
  ...
  public function processDefinition(&$definition, $plugin_id) {
    ...
    foreach (['id', 'label', 'entity_type'] as $required_property) {
      if (empty($definition[$required_property])) {
        throw new PluginException(sprintf('The fee "%s" must define the %s property.', $plugin_id, $required_property));
      }
    }
