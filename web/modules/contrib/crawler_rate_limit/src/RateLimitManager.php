...
class RateLimitManager implements RateLimitManagerInterface {
   ...
   * @var array
   */
  protected array $ipAddressBlocklist;
  ...
  public function __construct(CrawlerDetect $crawlerDetect, RateLimitBackendFactory $factory) {
    ...
    $this->ipAddressBlocklist = $settings['ip_address_blocklist'];
  ...
  public function block(Request $request) {
    if (!$this->isEnabled() || (empty($this->asnBlocklist) && empty($this->ipAddressBlocklist) && empty($this->countryBlockList))) {
      return FALSE;
    }

    $this->clientIp ??= $request->getClientIp();
    $this->clientAsn ??= $this->ipToAsn($this->clientIp);
    if (in_array($this->clientIp, $this->ipAddressBlocklist)) {
      return TRUE;
    }
